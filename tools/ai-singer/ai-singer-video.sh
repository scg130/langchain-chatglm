...
# 合并音频 同时保留视频和伴奏的声音
ffmpeg -i $VIDEO -i $ACCOMP -filter_complex "[0:a][1:a]amix=inputs=2:duration=shortest:dropout_transition=2[aout]" -map 0:v -map "[aout]" -c:v copy -c:a aac -shortest "${OUTDIR}/final_mv.mp4" -y
